//** 0.1초 단위 스톱워치 **//
// FND의 소수점 표시 기능을 이용하여
// 1/10초가 표시되는 (0.1초 단위) 스톱워치를 만들자

// 0부터 시작해 초를 세는 스톱워치 (단,0.1초 단위, 소수점 표기 필수)
// 0번 버튼 : 카운팅 시작
// 1번 버튼 : 카운팅 종료
// 2번 버튼 : 리셋
...
volatile int sec = 0, min = 0, hour = 0, msec = 0;
...
void display_fnd(int num, int num_2, int num_3) {
	...
	GPIO_SetBits(GPIOD, fnd[num2] | 0b1000000000000000);
	GPIO_SetBits(GPIOB, fnd_sel[2]);
	GPIO_ResetBits(GPIOD, fnd[num2] | 0b1000000000000000);
...
int main(void) {
	init();
	int fnd_min = 0, fnd_sec = 0, fnd_msec = 0;
	...
	for (;;) {
		//mode set
		if (mode == 0)
			display_fnd(fnd_msec, fnd_sec, fnd_min); //FND display(num)
		else if (mode == 1)
			display_fnd(msec, sec, min);

		// GPIOD0 입력시 GPIOB bit set, num++;
		...
		} else if ((GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_0) == 1)
				&& flag == 1) {
			flag = 0;
		}

		// GPIOD1 이력시 GPIOB bit set, num--;
		...
		} else if ((GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_1) == 1)
				&& flag2 == 1) {
			flag2 = 0;
		}
		// GPIOD2
		...
		//GPIOD3
		if ((GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_3) == 0) && flag4 == 0) {
