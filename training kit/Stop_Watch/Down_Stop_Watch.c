// 감소하는 스톱워치
// 0번 버튼 : 초 증가
// 1번 버튼 : 초 감소 (0아래인 경우 59가 되면서 분감소, 음수 예외처리)
// 2번 버튼 : 카운터 감소 시작
// 3번 버튼 : 카운터 감소 종료 - > 한번더 누르면 0으로 리셋
/**
 ******************************************************************************
 * @file    main.c
 * @author  Ac6
...
int main(void) {
	...
	for (;;) {
		...
		if ((GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_0) == 0) && flag == 0) {
			if (mode ==0){
			fnd_sec++;
			if (fnd_sec>=60){ fnd_sec=0;fnd_min++;
			if(fnd_min >=60) fnd_min=0;}
			}
			flag = 1;
			delay(10);
		...
		if ((GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_1) == 0) && flag2 == 0) {
			if(mode ==0){
			fnd_sec--;
			if (fnd_sec<0){ fnd_sec = 59; fnd_min--;
			if(fnd_min <0)fnd_min=59;}
			}
			flag2 = 1;
			delay(10);
